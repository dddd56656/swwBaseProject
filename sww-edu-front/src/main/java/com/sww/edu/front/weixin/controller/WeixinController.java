package com.sww.edu.front.weixin.controller;
...
import com.sww.edu.front.user.service.UserService;
import com.sww.edu.user.api.UserWeixinRemoteService;
import com.sww.edu.user.api.dto.WeixinDTO;
...
public class WeixinController {
    ...
    public ResponseDTO bind() {
        ...
        String redirectUrl = "http://edufront.sww.com/front/weixin/callback?isBind=true&uid=" + userIdBase64;
...
//        return "redirect:" + url;
    ...
    public String login() {
        String redirectUrl = "http://edufront.sww.com/front/weixin/callback";
        String url = wxMpService.buildQrConnectUrl(redirectUrl, WxConsts.QrConnectScope.SNSAPI_LOGIN, null);
            ...
            if (Boolean.TRUE.equals(isBind)) {
                ...
                if (null != weixin) {
                    if (StringUtils.isNotBlank(weixin.getOpenId()) && StringUtils.equals(weixin.getOpenId(), wxMpUser.getOpenId())) {
                        ...
                        return "redirect:http://edufront.sww.com/#/setting";
                    } else {
                        return "redirect:http://edufront.sww.com/#/setting?msg=" + URLUtil.encode("用户已绑定了其他微信账号");
                    }
                } else {
                    ...
                    if (null == weixinByOpenId) {
                        ...
                        if (null != result && !result.isSuccess()) {
                            return "redirect:http://edufront.sww.com/#/setting?msg=" + URLUtil.encode("该微信账号已被其他用户绑定");
                        }
                        return "redirect:http://edufront.sww.com/#/setting";
                    } else {
                        return "redirect:http://edufront.sww.com/#/setting?msg=" + URLUtil.encode("该微信账号已被其他用户绑定");
                ...
                return "redirect:http://edufront.sww.com/#/setting?rtoken=" + (StringUtils.isNotBlank(refreshToken) ? refreshToken : StringUtils.EMPTY);
            }
        } catch (Exception e) {
            log.error("", e);
        }
